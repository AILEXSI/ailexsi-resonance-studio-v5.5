...
| Deps / SBOM | Mediabunny **removed**. CycloneDX SBOMs + inventory: `docs/compliance/`. MODE A AFE-16: **1125 passed / 3 failed / 1128 total in 133 files** (`tsc --noEmit` clean). The 3 failures are AFE-15 A/N/O and fail identically on `main` (`visFrames`/`blackFrames` trip `/VIS|BLACK/` dump bans; O `flushCount` 1 not 2). AFE-16 file: **10/10**. `vite build` clean (vite 7.3.6, 143 modules, version 5.5.0). Chrome B-frame pixels **1188/1188 EXACT** remain AFE-04/05 evidence (`docs/compliance/afe-04-evidence-summary.json` + `afe-05-evidence-summary.json`). **No LICENSE. No THIRD_PARTY_NOTICES. Does not claim MPL FREE. Licensing is not HUMAN-PROVEN.** |
