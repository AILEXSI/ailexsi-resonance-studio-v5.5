...
| Deps / SBOM | Mediabunny **removed**. CycloneDX SBOMs + inventory: `docs/compliance/`. MODE A AFE-15: **1118 tests passed in 132 files**. `tsc --noEmit` clean. `vite build` clean (vite 7.3.6, 143 modules, version 5.5.0). Chrome B-frame pixels **1188/1188 EXACT** remain AFE-04/05 evidence (`docs/compliance/afe-04-evidence-summary.json` + `afe-05-evidence-summary.json`). **No LICENSE. No THIRD_PARTY_NOTICES. Does not claim MPL FREE. Licensing is not HUMAN-PROVEN.** |
