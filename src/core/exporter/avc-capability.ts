 ...
 *
 * Export used a fixed `avc1.42001f` (Baseline / Level 3.1). Level 3.1 maxes
 * out at 3600 macroblocks / 108000 MB/s — enough for 1280×720@30, not for
 * 1920×1080 (8160 MB, 244800 MB/s @30 → Level 4.0).
 *
 * Probe `VideoEncoder.isConfigSupported` from the minimum legal level upward.
 * No WebM fallback. No silent skip. No non-AVC substitution.
 */
...
/** ITU-T H.264 Table A-1 subset used for export size/fps. */
...
export async function selectAvcEncoderConfig(
  ...
  for (const codec of tried) {
    ...
    try {
      ...
      /* next candidate — never treat a probe throw as success */
    }
  }
