...
Vitest under `tests/`. Last full count on the accepted EXE SHA `234a781` (PR #15): **831 passed / 94 files** (vitest 3.2.7). F branch: **841 passed / 96 files**. G branch: **862 passed / 98 files** (`tests/core/volume-automation.test.ts` + `tests/layout/volume-automation.test.tsx`). H adds `tests/core/volume-write.test.ts` (operator list 1–25) plus W chrome in the G layout file. H tip MODE A: **887 passed / 99 files**. Test count ≠ HUMAN-PROVEN.
